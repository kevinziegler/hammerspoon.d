...
function showHelper(mappingInfo)
  local maxNameLength = 0
  local maxDescriptionLength = 0
  local mapSeparator = " → "

  local menus = ''
  local actions = ''

  table.sort(mappingInfo)

  for _, info in pairs(mappingInfo) do
    maxNameLength = math.max(maxNameLength, string.len(info.name))
    maxDescriptionLength = math.max(maxDescriptionLength, string.len(info.description))
  end

  function makeHeader(header)
    headerWidth = string.len(mapSeparator) + maxNameLength + maxDescriptionLength
    -- return header .. " " .. string.rep("—", headerWidth - string.len(header))
    return header .. " " .. string.rep("—", headerWidth - string.len(header) - 1)
  end

  function makeDisplayLine(info)
    if not info then
        return "" .. string.rep(" ", maxNameLength) .. "   " .. string.rep(" ", maxDescriptionLength)
    end
    return ""
      .. string.rep(" ", maxNameLength - string.len(info.name))
      .. info.name
      .. " → "
      .. info.description
      .. string.rep(" ", maxDescriptionLength - string.len(info.description))
  end

  actionItems = {}
  menuItems = {}

  for key, info in pairs(mappingInfo) do
    if info.action == "function" then
      actions = actions .. makeDisplayLine(info) .. "\n"
      actionItems[#actionItems + 1] = info
    else
      menus = menus .. makeDisplayLine(info) .. "\n"
      menuItems[#menuItems + 1] = info
    end
  end

  local hc = makeHeader("Actions") .. "    " .. makeHeader("More") .. "\n"
  local i = 1
  while actionItems[i] or menuItems[i] do
    hc = hc .. makeDisplayLine(actionItems[i]) .. "    " .. makeDisplayLine(menuItems[i]) .. "\n"
    i = i+1
  end
  ...
  if string.len(actions) > 0 then
    helperContent = helperContent .. makeHeader("Actions") .. actions .. "\n"
  end

  if string.len(menus) > 0 then
    helperContent = helperContent .. makeHeader("More") .. menus
  end

  -- previousHelperID = hs.alert.show(helperContent, obj.helperFormat, true)
  previousHelperID = hs.alert.show(hc, obj.helperFormat, true)
end
...
function modalAction(modal, func)
  return function()
      modal:exit()
      killHelper()
      if func then
...
function obj.recursiveBind(keymap)
   if type(keymap) == 'function' then
      ...
      return keymap
   elseif keymap.generate then
     return keymap.generate
   end
   ...
   for key, map in pairs(keymap) do
      ...
      local info = {
        name = createKeyName(key),
        description = key[3],
        action = type(map)
      }

      mappingInfo[info["name"]] = info

      modal:bind(key[1], key[2], modalAction(modal, func))
      modal:bind(obj.escapeKey[1], obj.escapeKey[2], modalAction(modal, nil))
   end
